// EB01-039 Conquerer of Three Worlds Ragnaraku (event).
...
const EB01_039 = ALL_CARDS.find(c => c.id === 'EB01-039')!;
...
describe('EB01-039 — Conquerer of Three Worlds Ragnaraku', () => {
  const clause = EB01_039.effectSpecV2!.clauses![0];

  it('cost: DON-1 from cost-area to deck', () => {
    const s = boot();
  ...
  it('target excludes cost-9 opp char', () => {
    const s = boot();
    placeOppChar(s, 'big', 9);
    const ids = resolveTargetV2(s, 'A', 'src', clause.target);
    expect(ids).not.toContain('big');
  });

  it('action KOs a cost-8 opp char', () => {
    const s = boot();
    placeOppChar(s, 'c8', 8);
    applyActionV2(s, { sourceInstanceId: 'src', controller: 'A' }, clause.action, ['c8']);
    expect(s.players.B.field.some((i: { instanceId: string }) => i.instanceId === 'c8')).toBe(false);
    expect(s.players.B.trash).toContain('c8');
  });
});

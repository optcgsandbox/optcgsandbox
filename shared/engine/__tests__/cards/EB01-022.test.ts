// EB01-022 Inazuma.
//   "[End of Your Turn] If you have 2 or less cards in your hand, draw
//    2 cards."
...
const EB01_022 = ALL_CARDS.find(c => c.id === 'EB01-022')!;
...
describe('EB01-022 — Inazuma', () => {
  const clause = EB01_022.effectSpecV2!.clauses![0];

  it('condition TRUE when hand has 2 cards', () => {
    const s = boot();
    s.players.A.hand = s.players.A.hand.slice(0, 2);
    expect(evaluateConditionV2(s, 'A', clause.condition, 'src')).toBe(true);
  });

  it('condition TRUE when hand has 0 cards', () => {
    const s = boot();
    s.players.A.hand = [];
    expect(evaluateConditionV2(s, 'A', clause.condition, 'src')).toBe(true);
  });

  it('condition FALSE when hand has 3 cards', () => {
    const s = boot();
    while (s.players.A.hand.length < 3) s.players.A.hand.push('x');
    s.players.A.hand = s.players.A.hand.slice(0, 3);
    expect(evaluateConditionV2(s, 'A', clause.condition, 'src')).toBe(false);
  });

  it('action draws 2 cards', () => {
    const s = boot();
    s.players.A.hand = s.players.A.hand.slice(0, 2);

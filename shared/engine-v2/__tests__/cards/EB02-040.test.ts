/**
 * Per-card semantic test — EB02-040 BRAND NEW WORLD ([Main] event).
 ...
 * Same effect family as EB02-008/020/031.
 *
...
const L: LeaderCard = {
  id: 'TEST_L_EB02040', name: 'L', kind: 'leader', colors: ['purple'], cost: null,
  power: 5000, life: 5, counterValue: null, traits: [], keywords: [], effectTags: [],
};

function deckChar(id: string, cost: number): CharacterCard {
  return {
    id, name: id, kind: 'character', colors: ['purple'], cost, power: 1000,
    counterValue: 1000, traits: [], keywords: [], effectTags: [],
  };
}

describe('EB02-040 — BRAND NEW WORLD', () => {
  const c = loadCards().find((x) => x.id === 'EB02-040');
  if (c === undefined || c.kind !== 'event') throw new Error('EB02-040 invalid');
  ...
  it('cost-4 in top 4 → added to hand', () => {
    const cand = deckChar('TEST_C4_E40', 4);
    const fillers = [deckChar('TEST_F1_E40', 1), deckChar('TEST_F2_E40', 1), deckChar('TEST_F3_E40', 1)];
  ...
  it('cost-3 in top 4 → NOT added (boundary)', () => {
    const low = deckChar('TEST_C3_E40', 3);
    const fillers = [deckChar('TEST_FA_E40', 1), deckChar('TEST_FB_E40', 1), deckChar('TEST_FC_E40', 1)];
    const { state } = buildState({ leaderA: L });
  ...
  it(
    ...
    () => {
      const cand = deckChar('TEST_PICK_E40', 4);
      const fillers = [deckChar('TEST_LO1_E40', 1), deckChar('TEST_LO2_E40', 1), deckChar('TEST_LO3_E40', 1)];
      const tail = deckChar('TEST_TAIL_E40', 1);
      const { state } = buildState({ leaderA: L });

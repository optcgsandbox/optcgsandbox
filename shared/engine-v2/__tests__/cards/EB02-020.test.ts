/**
 * Per-card semantic test — EB02-020 We Are! ([Main] event).
 ...
 * Spec: same as EB02-008 The Peak: on_play / searcher_peek lookCount:4
 *   addCount:1 filter{costMin:4}.
 *
 * Engine gap re-ref EB01-009 / EB02-008: searcher_peek leftover-to-bottom
 *   goes to TOP. Leftover-bottom assertion uses it.fails.
 */
...
const L: LeaderCard = {
  id: 'TEST_L_EB02020', name: 'L', kind: 'leader', colors: ['green'], cost: null,
  power: 5000, life: 5, counterValue: null, traits: [], keywords: [], effectTags: [],
};

function deckChar(id: string, cost: number): CharacterCard {
  return {
    id, name: id, kind: 'character', colors: ['green'], cost, power: 1000,
    counterValue: 1000, traits: [], keywords: [], effectTags: [],
  };
}

describe('EB02-020 — We Are!', () => {
  const c = loadCards().find((x) => x.id === 'EB02-020');
  if (c === undefined || c.kind !== 'event') throw new Error('EB02-020 invalid');
  ...
  it('cost-4 in top 4 → added to hand', () => {
    const cand = deckChar('TEST_C4_E20', 4);
    const fillers = [deckChar('TEST_F1_E20', 1), deckChar('TEST_F2_E20', 1), deckChar('TEST_F3_E20', 1)];
  ...
  it('cost-3 in top 4 → NOT added (boundary exclusive)', () => {
    const low = deckChar('TEST_C3_E20', 3);
    const fillers = [deckChar('TEST_F1B_E20', 1), deckChar('TEST_F2B_E20', 1), deckChar('TEST_F3B_E20', 1)];
    const { state } = buildState({ leaderA: L });
  ...
  it(
    'leftover top-4 non-picked cards go to BOTTOM (closes cluster-A engine gap)',
    () => {
      const cand = deckChar('TEST_PICK_E20', 4);
      const fillers = [deckChar('TEST_LO1_E20', 1), deckChar('TEST_LO2_E20', 1), deckChar('TEST_LO3_E20', 1)];
      const tail = deckChar('TEST_TAIL_E20', 1);
      const { state } = buildState({ leaderA: L });

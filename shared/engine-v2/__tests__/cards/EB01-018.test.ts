/**
 * Per-card semantic test — EB01-018 Mountain God (vanilla character).
 *
 * Printed text (cards.json): "-" (no printed effect)
 * Stats: 5 cost / 7000 power / 1000 counter / Animal + Land of Wano / green
 */
...
describe('EB01-018 — Mountain God (vanilla character)', () => {
  ...
  const card = allCards.find((c) => c.id === 'EB01-018');
  if (card === undefined) throw new Error('EB01-018 not in cards.json');

  it('is a character', () => {
    expect(card.kind).toBe('character');
  });

  it('printed stats: cost 5 / power 7000 / counter 1000', () => {
  ...
  it('green color + Animal + Land of Wano traits', () => {
    expect(card.colors).toContain('green');
    expect(card.traits).toContain('Animal');
    expect(card.traits).toContain('Land of Wano');
  });

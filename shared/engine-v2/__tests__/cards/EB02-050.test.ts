/**
 * Per-card semantic test — EB02-050 Kokoro no Chizu ([Main] event).
 ...
 * Same effect family as EB02-008/020/031/040.
 *
 * Engine gap re-ref EB02-008/EB01-009: searcher_peek leftover unshifts to
 *   TOP not BOTTOM. Leftover-bottom assertion uses it.fails.
 */
...
const L: LeaderCard = {
  id: 'TEST_L_EB02050', name: 'L', kind: 'leader', colors: ['black'], cost: null,
...
describe('EB02-050 — Kokoro no Chizu', () => {
  const c = loadCards().find((x) => x.id === 'EB02-050');
  if (c === undefined || c.kind !== 'event') throw new Error('EB02-050 invalid');
  ...
  it('cost-4 in top 4 → added to hand', () => {
    const cand = deckChar('TEST_C4_E50', 4);
    const fillers = [deckChar('TEST_F1_E50', 1), deckChar('TEST_F2_E50', 1), deckChar('TEST_F3_E50', 1)];
  ...
  it('cost-3 in top 4 → NOT added (boundary)', () => {
    const low = deckChar('TEST_C3_E50', 3);
    const fillers = [deckChar('TEST_FA_E50', 1), deckChar('TEST_FB_E50', 1), deckChar('TEST_FC_E50', 1)];
    const { state } = buildState({ leaderA: L });
  ...
  it(
    ...
    () => {
      const cand = deckChar('TEST_PICK_E50', 4);
      const fillers = [deckChar('TEST_LO1_E50', 1), deckChar('TEST_LO2_E50', 1), deckChar('TEST_LO3_E50', 1)];
      const tail = deckChar('TEST_TAIL_E50', 1);
      const { state } = buildState({ leaderA: L });
